...
from .safeplug.model.language_model.safeplug_llama import (LlavaLlamaForCausalLM,
                                                     LlavaLlamaModel)
...
class SafePLUGMetaModel:
    def __init__(
        self,
        config,
        **kwargs,
    ):
        super(SafePLUGMetaModel, self).__init__(config)

        self.config = config
        # self.config.train_mask_decoder = kwargs["train_mask_decoder"]
        # self.config.out_dim = kwargs["out_dim"]
        self.vision_pretrained = kwargs.get("vision_pretrained", None)
        self.initialize_lisa_modules(self.config, **kwargs)

    def initialize_lisa_modules(self, config, **kwargs):
        # SAM
        self.visual_model = build_sam_vit_h(self.vision_pretrained, image_size=kwargs.get("sam_img_size", 1024))
...
class SafePLUGModel(SafePLUGMetaModel, LlavaLlamaModel):
    def __init__(
        self,
        config,
        **kwargs,
    ):
        super(SafePLUGModel, self).__init__(config, **kwargs)
        ...
        if 'pretrain_mm_mlp_adapter' not in kwargs:
            ...
            self.config.pretrain_mm_mlp_adapter = kwargs['pretrain_mm_mlp_adapter']
        self.config.mm_use_im_patch_token = False
        if 'image_tower' in kwargs:
            self.config.image_tower = kwargs['image_tower']
        if 'video_tower' in kwargs:
            self.config.video_tower = kwargs['video_tower']


class SafePLUGForCausalLM(LlavaLlamaForCausalLM):
    def __init__(
        self,
        config,
        **kwargs,
        ...
        self.model = SafePLUGModel(config, **kwargs)
    ...
    def get_visual_embs(self, pixel_values: torch.FloatTensor):
        with torch.no_grad():
            ...
            for i in range(pixel_values.shape[0]):
                ...
                image_embeddings = self.model.visual_model.image_encoder(  # SAM is visual model，vision tower is CLIP
    ...
    def model_forward(
        ...
        images_bind: Optional[torch.FloatTensor] = None,
        ...
        **kwargs,
        ...
        Args:
            images (torch.FloatTensor): Input images with shape (batch_size, channels, height, width) for SAM
            images_bind (torch.FloatTensor): Input images with shape (batch_size, channels, height, width) for LanguageBind
        ...
        if seg_flag:
            # [b, 256, 64, 64]
            image_embeddings = self.get_visual_embs(images_sam)  # [b, c, h, w] --> [b, 256, 64, 64]
        ...
        # B*q_num, 255+sequence_length, 256  --> B*q_num, 256   
        # extract all [256] vectors from last_hidden_state where seg_token_mask == True; length is the total number of elements in seg_token_mask that are True (i.e., the number of selected tokens).
        pred_embeddings = last_hidden_state[seg_token_mask]
        # because there is only one seg token for each answer, so the length of pred_embeddings is B*q_num; and because it is single-turn conversation, so B*q_num is batch_size
        ...
        for i in range(len(pred_embeddings)):
            ...
            (
                ...
                boxes=None,
                masks=None,
                text_embeds=pred_embeddings[i].unsqueeze(0).unsqueeze(1),  # [1, 1, 256]
            )
            ...
            # low_res_masks: [1, 1, 256, 256], resize_list[i]: [685, 1024], label_list[i]: [h, w]
            # pred_mask: [1, 1, h, w]
            ...
            pred_masks.append(pred_mask[:, 0])  # [1, h, w]
            # iou_predictions: [1,1]
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            # if True:
                ...
                last_hidden_state = torch.stack(hidden_states, dim=-1).sum(dim=-1)  # [1, 1, 256, list_len] --> [1, 1, 256]
                # B*q_num, N, 256 --> B*q_num, 255+sequence_length, 256
                # last_hidden_state = last_hidden_state[:, :seg_token_mask.shape[1], :]
                pred_embeddings = last_hidden_state[seg_token_mask]
                # pred_embeddings = last_hidden_state
