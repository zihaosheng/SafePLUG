...
class ModelWorker:
    ...
    def generate_stream(self, params):
        ...
        print("==========>region_masks", len(region_masks))
        for region_mask_i in region_masks:
            print("==========>region_mask_i.shape", np.array(region_mask_i).shape)
        ...
        if images is not None and len(images) > 0 and self.is_multimodal:
            if len(images) > 0:
                ...
                #------------preprocess image for bind  ------------
                image_bind = [load_image_from_base64(image) for image in images][0]
                image_bind = expand2square(image_bind, tuple(int(x * 255) for x in image_processor.image_mean))
                image_bind = image_processor.preprocess(image_bind, return_tensors='pt')['pixel_values'][0]
        ...
        if region_masks is not None and len(region_masks) > 0:
        ...
        if self.precision == "fp16":
            image_bind = image_bind.unsqueeze(0).half()
            image_sam=image_sam.unsqueeze(0).half()
        elif self.precision == "bf16":
            image_bind = image_bind.unsqueeze(0).bfloat16()
            image_sam=image_sam.unsqueeze(0).bfloat16()
        elif self.precision == "fp32":
            image_bind = image_bind.unsqueeze(0).float()
            image_sam=image_sam.unsqueeze(0).float()
        elif self.precision == "int8":
            image_bind = image_bind.unsqueeze(0).int()
        ...
        if self.args.device_map == 'cuda':
            image_bind = image_bind.to("cuda")
            image_sam=image_sam.to("cuda")
            input_ids = input_ids.to("cuda")
            region_masks = torch.tensor([region_masks]).cuda() if region_masks is not None else None
...
# ----------------------------------return all token once---------------------------------------------
        ...
        #     # print("Length of encoded_mask:", len(encoded_mask))
        # else:
        #     encoded_mask = []
        #     height = width = 0
...
# ----------------------------------return token by token---------------------------------------------
        ...
        for i in range(max_new_tokens):
            if i == 0:
                ...
                attention_mask = torch.ones_like(input_ids, device=self.args.device_map)
...
if __name__ == "__main__":
    ...
    if args.multi_modal:
        logger.warning("Multimodal mode is automatically detected with model name, please make sure `model.safeplug` is included in the model path.")
