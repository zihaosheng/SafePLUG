...
python -m model.safeplug.serve.gradio_web_server --controller http://localhost:10000 --add_region_feature
'''
import argparse
import datetime
import json
...
from model.safeplug.conversation_demo import (default_conversation, conv_templates,
                                   SeparatorStyle)
from model.safeplug.constants import LOGDIR
            ...
            if chunk:
                ...
                if data["error_code"] == 0:
                    if 'mask' in data and len(data['mask']) > 0:
                        ...
                        # print("shape:", shape)
                        # print("mask data:", data['mask'])
                        mask = decode_sparse(data['mask'], shape)
                        # print("mask after decode:", mask)
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(title="SafePLUG", theme=gr.themes.Base(), css=css) as demo:
        state = gr.State()
        state.skip_next = False
        cur_dir = os.path.dirname(os.path.abspath(__file__))

        if not embed_mode:
            logo_path = os.path.join(cur_dir, "examples", "logo.png")
            ...
            <div style='display: flex; align-items: center;justify-content: center;'>
                <img src="data:image/png;base64,{encoded_logo}" style='width: 60px; height: 60px; border: none;' alt='Icon' />
        ...
        with gr.Row():
            with gr.Column(scale=4):
                ...
                sketch_pad = gr.ImageMask(label="Video/Image & Sketch", type="pil", elem_id="img2text")
                ...
                gr.Examples(examples=[
                    [f"{cur_dir}/examples/mm-au-1092-1_original.png", "Give a comprehensive description of the object or region marked as <region> in this driving scenario."],
                    [f"{cur_dir}/examples/dota-PEwiwzyTjX0_001311-scene-grounding_000051.jpg", "Segment the area that could lead to 'a collision between two vehicles traveling in the same direction, where one vehicle moves laterally into the path of another.'"],
                    [f"{cur_dir}/examples/dota-LfKfK4I5RPE_004721-scene-grounding_000060.jpg", "Based on the phrase 'a collision that occurred when a vehicle turned into or crossed the path of another vehicle.', segment the relevant region that could lead to it."],
            ...
            with gr.Column(scale=5):
                chatbot = gr.Chatbot(elem_id="chatbot", label="SafePLUG", visible=False).style(height=750)
                with gr.Row():
