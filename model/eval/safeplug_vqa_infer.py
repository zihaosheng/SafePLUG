...
def main(args):
    ...
    model.eval()
    if args.eval_seg:
        validate_seg(val_loader, model, 0, args, tokenizer)
        exit()
    if args.eval_vqa:
        validate_vqa(val_loader, model, 0, args, tokenizer)
        exit()
...
def vis_overlay_masks(original_image_path, prediction_mask, ground_truth_mask, save_path):
    ...
    cv2.imwrite(save_path.replace('.png', '_ori.png'), cv2.cvtColor(original_image, cv2.COLOR_RGB2BGR))
...
def validate_seg(val_loader, model_engine, epoch, args, tokenizer):
    intersection_meter = AverageMeter("Intersec", ":6.3f", Summary.SUM)
    union_meter = AverageMeter("Union", ":6.3f", Summary.SUM)
    acc_iou_meter = AverageMeter("gIoU", ":6.3f", Summary.SUM)
    iou_meter = AverageMeter("IoU", ":.6f", Summary.SUM)
    dice_meter = AverageMeter("Dice", ":.6f", Summary.SUM)
    ap30_meter = AverageMeter("AP@30", ":.6f", Summary.SUM)
    ap50_meter = AverageMeter("AP@50", ":.6f", Summary.SUM)
    ap70_meter = AverageMeter("AP@70", ":.6f", Summary.SUM)
    modality_metric = {}
    ...
    for input_dict in pbar:
        ...
        if modality not in modality_metric.keys():
            ...
            modality_metric[modality]['ap30'] = []
            modality_metric[modality]['ap50'] = []
            modality_metric[modality]['ap70'] = []

        if not args.cpu_only:
            torch.cuda.empty_cache()
            input_dict = dict_to_cuda(input_dict)
        if args.precision == "fp16":
            input_dict["images_sam"] = input_dict["images_sam"].half()
            input_dict["images_bind"] = [i.half() for i in input_dict["images_bind"]]
        elif args.precision == "bf16":
            input_dict["images_sam"] = input_dict["images_sam"].bfloat16()
            input_dict["images_bind"] = [i.bfloat16() for i in input_dict["images_bind"]]
        else:
            input_dict["images_sam"] = input_dict["images_sam"].float()
            input_dict["images_bind"] = [i.float() for i in input_dict["images_bind"]]
        ...
        with torch.no_grad():
            ...
            input_dict["images_bind"],
            input_dict["images_sam"],
            input_ids,
        ...
        ap30_value = 1 if iou > 0.3 else 0
        ap50_value = 1 if iou > 0.5 else 0
        ap70_value = 1 if iou > 0.7 else 0
                
        ap30_meter.update(ap30_value)
        ap50_meter.update(ap50_value)
        ap70_meter.update(ap70_value)
        ...
        modality_metric[modality]['ap30'].append(ap30_value)
        modality_metric[modality]['ap50'].append(ap50_value)
        modality_metric[modality]['ap70'].append(ap70_value)

        new_description = f'iou={iou:.6f} '
        pbar.set_description(new_description)
        idx += 1
        if args.vis_mask:
            model_name = args.version.split('/')[-1]
            test_file_name = args.val_data_path.split('/')[-1].split('.')[0]
            img_name = input_dict['image_paths'][0].split('/')[-3] + '_' + os.path.basename(input_dict['image_paths'][0])[:-4]
    ...
    print("ap30: {:.6f}, ap50: {:.6f}, ap70: {:.6f}".format(ap30, ap50, ap70))
    ...
    return miou, mDice, ap30, ap50, ap70
