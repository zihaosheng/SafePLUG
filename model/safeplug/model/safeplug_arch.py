...
def point_sample(input, point_coords, return_dtype, **kwargs):
    """
    # get the value of input at the coordinates of point_coords
    ...
    if point_coords.dim() == 3:
        add_dim = True
        point_coords = point_coords.unsqueeze(2)  # change the dimension of point_coords from (N, P, 2) to (N, P, 1, 2)
    # output = F.grid_sample(input, 2.0 * point_coords - 1.0, **kwargs)
    output = F.grid_sample(input.float(), (2.0 * point_coords - 1.0).float(), **kwargs)
    output = output.to(return_dtype)
    if add_dim:
        output = output.squeeze(3)  # change the dimension of output from (N, C, P, 1) to (N, C, P)
    return output
...
class LlavaMetaModel:
    def __init__(self, config):
        ...
        # project to the dimension of visual prompt
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if image_tower is not None:
            if self.get_image_tower() is None:
                ...
                print("Image tower is build successfully.")
        ...
        if video_tower is not None:
            if self.get_video_tower() is None:
                ...
                print("Video tower is build successfully.")
        ...
        print('load image_tower and video_tower successfully!!!')
...
class LlavaMetaForCausalLM(ABC):
    ...
    #     image_features = self.get_model().get_vision_tower()(images)  # 得到image_features
    #     projected_image_features = self.get_model().mm_projector(image_features)  # 投影到LLM空间
    ...
    def encode_images(self, images, region_flag=False, region_geo_sampler=False):
        image_features = self.get_model().get_image_tower()(images)  # get image_features
        projected_image_features = self.get_model().mm_projector(image_features)  # project to the dimension of LLM
    ...
    def encode_videos(self, videos):  # [mini_b, c, t, h, w]
        ...
        video_features = self.get_model().get_video_tower()(videos)  # [mini_b, t, n, c]: [2, 8, 257, 1024]
        video_features = self.get_model().mm_projector(video_features)  # [mini_b, t, n, c]: [2, 8, 257, 4096]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        """
        input_ids: [batch_size, seq_len] [2, 722]
        attention_mask: [batch_size, seq_len] [2, 722]
        past_key_values: [batch_size, num_layers, 2, seq_len, d_model] Here  it is None
        labels: [batch_size, seq_len] [2, 722]
        images: list of tensor
        region_masks: list
        valid_region_masks_bool: list
        """
        ...
        if (image_tower is None and video_tower is None) or images is None or input_ids.shape[1] == 1:
            ...
            return input_ids, attention_mask, past_key_values, None, labels  # compared to Video-LLaVA, here we don't return position_ids
        ...
        if getattr(images_minibatch, 'ndim', 0) == 4:  # batch consists of images, [mini_b, c, h, w]
            if image_tower is not None:
                raw_image_features, image_features_minibatch, region_feature_map = self.encode_images(images_minibatch, region_flag, region_geo_sampler)  # [mini_b, l, c]
            else:
        ...
        if getattr(videos_minibatch, 'ndim', 0) == 5:  # batch consists of videos, [mini_b, c, t, h, w]
            video_features_minibatch = self.encode_videos(videos_minibatch)  # fake list [mini_b, t, l, c]: [2, 8, 257, 4096]
        ...
        image_features = new_tmp  # flatten the videos along the time dimension
        ...
        if region_flag:
            valid_region_masks_bool_image = [valid_region_masks_bool[idx] for idx in image_idx]   
            valid_region_masks_bool_image = torch.tensor([any(item) for item in valid_region_masks_bool_image])
            region_feature_map = region_feature_map[valid_region_masks_bool_image]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            if (cur_input_ids == IMAGE_TOKEN_INDEX).sum() == 0:
                ...
                # current sample has no image token, only text
                ...
                #     cur_input_embeds
                #     + (
                #         0.0 * self.get_model().mm_projector(image_tower.dummy_feature)
                #     ).sum()
                # )
            ...
            image_token_indices = torch.where(cur_input_ids == IMAGE_TOKEN_INDEX)[0]  # find the indices of all image tokens
            ...
            while image_token_indices.numel() > 0:
                ...
                if region_flag:
                    assert (cur_input_ids[:image_token_start] == REGION_TOKEN_INDEX).sum() == 0  # region token cannot appear before image token

                # If not use start-end token, pt ckpt saved only has mm projector.
                # ------------------------------------------------------------------------
                # ---------------------- Case 1: use start-end token ---------------------
                # ------------------------------------------------------------------------
                # if start-end token is used and tune_mm_mlp_adapter is used, the image token and text token need to be processed separately
                # first concatenate the text embedding before the image token, then concatenate the start token, then concatenate the image feature, 
                # then concatenate the end token.
                # for the label part, use IGNORE_INDEX to fill the image feature part.
                if getattr(self.config, "tune_mm_mlp_adapter", False) and getattr(
                    ...
                    cur_new_input_embeds.append(cur_image_features)  # concatenate the image feature
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(
                    ...
                    if labels is not None:
                        cur_new_labels.append(cur_labels[:image_token_start])  # concatenate the text embedding before the image token
                        ...
                        cur_new_labels.append(  # concatenate the image token
                    ...
                    cur_new_input_embeds.append(cur_image_features)  # concatenate the image feature [257, 4096]
                    cur_new_input_embeds.append(
                        self.get_model().embed_tokens(
                            cur_input_ids[image_token_start + 1 : image_token_start + 2]  # maybe because the embed_tokens received -200, the potential solution is to add <im_start> and <im_end> to each <image> in the video
                        )
                    )
                    if labels is not None:
                        cur_new_labels.append(cur_labels[:image_token_start])    # use the solution above to solve the error
                        cur_new_labels.append(
                            torch.full(
                                (cur_image_features.shape[0],),
                                IGNORE_INDEX,
            ...
            if cur_input_ids.numel() > 0:
                ...
                if region_flag and valid_region_masks_bool[batch_idx].item():
                    for idx,indice in enumerate(region_indices):  # here we assume that each image has only one region mask
                    ...
                    if region_flag:
                        assert valid_region_masks_bool[batch_idx].item() == False
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            for cur_new_embed in new_input_embeds:
                cur_new_embed = torch.cat(  # align the length, right pad with 0
                    (
                        cur_new_embed,
                        torch.zeros(
