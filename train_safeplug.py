...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        default="/data/huggingface-models/Video-LLaVA-Pretrain-7B/mm_projector.bin",
        type=str,
    )
    parser.add_argument(
        "--precision",
    ...
    parser.add_argument(
        "--sft_modules",
        default="lm_head,embed_tokens,mask_decoder,text_hidden_fcs",
        type=str,
        help="set the supervised fintuning modules",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="pad",
        help="How to handle image aspect ratio.",
    )
    parser.add_argument(
        "--is_multimodal",
        type=bool,
        default=True,
        help="Whether to use multimodal data.",
    )
    parser.add_argument(
        "--data_path",
        type=str,
        default="/path/to/xxx.json",
    ...
    parser.add_argument(
        "--use_mm_start_end", action="store_true", default=True
    )
...
def main(args):
    ...
    print("Initialize_vision_modules done!!!!!!!!!!")
    ...
    if not args.eval_only:
        ...
        print("Initialize_lisa_modules done!!!!!!!!!!")
    ...
    if args.sft_modules != "":
        ...
        for n, p in model.named_parameters():
            if any([x in n for x in sft_modules]):
    ...
    #         if param.requires_grad:
    #             print(
    #                 f"Parameter Name: {name}, Shape: {param.shape}, grad: {param.requires_grad}"
    #             )
    #         # print(param)
    ...
    args.steps_per_epoch = math.ceil(
        math.ceil(len(train_dataset) / (args.batch_size * torch.cuda.device_count()))
...
def train(
    ...
    if model.global_steps % args.steps_per_epoch > 0:
        print(
            "skipping first {} steps, global step is {}".format(
                model.global_steps % args.steps_per_epoch, model.global_steps
            )
        )

        for local_step in tqdm.tqdm(
            range(0, model.global_steps % args.steps_per_epoch)
        ):
    ...
    for local_step in range(
        model.global_steps % args.steps_per_epoch, args.steps_per_epoch
    ):
        for i in range(args.grad_accumulation_steps):
            try:
                input_dict = next(train_iter)
            ...
            if args.precision == "fp16":
                input_dict["images_sam"] = input_dict["images_sam"].half() if input_dict["images_sam"] is not None else None
                input_dict["images_bind"] = [i.half() for i in input_dict["images_bind"]]
            elif args.precision == "bf16":
                input_dict["images_sam"] = input_dict["images_sam"].bfloat16() if input_dict["images_sam"] is not None else None
                input_dict["images_bind"] = [i.bfloat16() for i in input_dict["images_bind"]]
            else:
                input_dict["images_sam"] = input_dict["images_sam"].float() if input_dict["images_sam"] is not None else None
                input_dict["images_bind"] = [i.float() for i in input_dict["images_bind"]]
            ...
            if input_dict["seg_flag"]:
                mask_bce_losses.update(
                    mask_bce_loss.item(), input_dict["images_sam"].size(0)
                )
                mask_dice_losses.update(
                    mask_dice_loss.item(), input_dict["images_sam"].size(0)
                )
                mask_losses.update(mask_loss.item(), input_dict["images_sam"].size(0))
                unscale_mask_bce_losses.update(
                    unscale_mask_bce_loss.item(), input_dict["images_sam"].size(0)
                )
                unscale_mask_dice_losses.update(
                    unscale_mask_dice_loss.item(), input_dict["images_sam"].size(0)
                )
                unscale_mask_losses.update(
                    unscale_mask_loss.item(), input_dict["images_sam"].size(0)
                )
                unscale_mask_iou_losses.update(
                    unscale_mask_iou_loss.item(), input_dict["images_sam"].size(0)
                )
                unscale_mask_focal_losses.update(
                    unscale_mask_focal_loss.item(), input_dict["images_sam"].size(0)
                )
        ...
        if local_step % args.print_freq == 0:
            ...
            if args.local_rank == 0:
                ...
                writer.add_scalar(
                    "train/mask_loss", mask_losses.avg, model.global_steps
                )
                writer.add_scalar(
                    "train/unscale_mask_bce_losses",
                    unscale_mask_bce_losses.avg,
                    model.global_steps,
                )
                writer.add_scalar(
                    "train/unscale_mask_dice_losses",
                    unscale_mask_dice_losses.avg,
                    model.global_steps,
                )
                writer.add_scalar(
                    "train/unscale_mask_losses",
                    unscale_mask_losses.avg,
                    model.global_steps,
                )
                writer.add_scalar(
                ...
                writer.add_scalar(
                    "train/unscale_mask_iou_losses",
                    unscale_mask_iou_losses.avg,
                    model.global_steps,
                )
                writer.add_scalar(
                    "train/unscale_mask_focal_losses",
                    unscale_mask_focal_losses.avg,
                    model.global_steps,
                )
