...
def DataCollatorForSupervisedDataset(list_data_dict: Sequence[Dict], inference: bool = False) -> Dict[str, torch.Tensor]:
    ...
    if max_mask_nums > 0:
        ...
        for idx, instance in enumerate(list_data_dict):
            if len(instance['masks'])>0:
                ...
                resize_list.extend(instance['resize'])  # [576, 1024] for SAM's resize
    ...
    for data_dict in list_data_dict:
        ...
        images_sam_list.append(data_dict.get('image_sam', None))
        # images_bind_list.append(data_dict.get('image_bind', None))
    ...
    # ==========================================================================================================
        ...
        '''
    if 'image_bind' in list_data_dict[0]:
        images_bind_list = [instance['image_bind'] for instance in list_data_dict]

        # adapt to multi-video or multi-image or multi-image & video
        new_images = []
        for image in images_bind_list:
            if type(image) is list:
                for i in image:
                    new_images.append(i)
            else:
                new_images.append(image)
        images_bind_list = new_images
    else:
        raise ValueError(f'image_bind not in {list_data_dict[0]}')


    final_batch = {
            "image_paths": image_path_list,
            "images_sam": None if any(i is None for i in images_sam_list) else torch.stack(images_sam_list, dim=0),  # TODO: currently, if there are videos in the batch, then images_sam will be None
            "images_bind": images_bind_list,
            "input_ids": batch['input_ids'],
            "labels": batch['labels'],  # the answer of the text
            "attention_masks": batch['attention_mask'],
            "masks_list": batch['masks'],
            "label_list": batch['label_list'],  # the label of the seg mask
